...
static struct physfs_state {
/* General PhysicsFS state ...*/
	int initialized = 0;
	ErrState *errorStates = NULL;
	DirHandle *searchPath = NULL;
	DirHandle *writeDir = NULL;
	FileHandle *openWriteList = NULL;
	FileHandle *openReadList = NULL;
	char *baseDir = NULL;
	char *userDir = NULL;
	char *prefDir = NULL;
	int allowSymLinks = 0;
	PHYSFS_Archiver **archivers = NULL;
	PHYSFS_ArchiveInfo **archiveInfo = NULL;
	volatile size_t numArchivers = 0;
...
static PHYSFS_Io *handleIo_duplicate(PHYSFS_Io *io)
{
    ...
    if (newfh->forReading)
    {
        newfh->next = state.openReadList;
        state.openReadList = newfh;
    } /* if */
    else
    {
        newfh->next = state.openWriteList;
        state.openWriteList = newfh;
    } /* else */
...
static ErrState *findErrorForCurrentThread(void)
{
    ...
    if (state.errorStates != NULL)
    {
        tid = __PHYSFS_platformGetThreadID();

        for (i = state.errorStates; i != NULL; i = i->next)
        {
            if (i->tid == tid)
            {
                if (errorLock != NULL)
...
void PHYSFS_setErrorCode(PHYSFS_ErrorCode errcode)
{
    ...
    if (err == NULL)
    {
        ...
        err->next = state.errorStates;
        state.errorStates = err;
...
static void freeErrorStates(void)
{
    ...
    for (i = state.errorStates; i != NULL; i = next)
    {
        next = i->next;
        allocator.Free(i);
    } /* for */
...
static DirHandle *openDirectory(PHYSFS_Io *io, const char *d, int forWriting)
{
    ...
    if (ext != NULL)
    {
        ...
        for (i = state.archivers; (*i != NULL) && (retval == NULL) && !claimed; i++)
        {
...
static char *calculateBaseDir(char *argv0)
{
...
{
    BAIL_IF(state.initialized, PHYSFS_ERR_IS_INITIALIZED, 0);
    ...
    state.baseDir = calculateBaseDir(argv0);
    if (!state.baseDir) goto initFailed;

    state.userDir = __PHYSFS_platformCalcUserDir();
    if (!state.userDir) goto initFailed;

    /* Platform layer is required to append a dirsep. */
    assert(state.baseDir[strlen(state.baseDir) - 1] == __PHYSFS_platformDirSeparator);
    assert(state.userDir[strlen(state.userDir) - 1] == __PHYSFS_platformDirSeparator);
...
static void freeSearchPath(void)
{
    ...
    closeFileHandleList(&state.openReadList);

    if (state.searchPath != NULL)
    {
        for (i = state.searchPath; i != NULL; i = next)
        {
            next = i->next;
            freeDirHandle(i, state.openReadList);
        } /* for */
...
{
    const size_t len = (state.numArchivers - idx) * sizeof (void *);
    PHYSFS_ArchiveInfo *info = state.archiveInfo[idx];
    PHYSFS_Archiver *arc = state.archivers[idx];

    /* make sure nothing is still using this archiver */
    if (archiverInUse(arc, state.searchPath) || archiverInUse(arc, state.writeDir))
    ...
    memmove(&state.archiveInfo[idx], &state.archiveInfo[idx+1], len);
    memmove(&state.archivers[idx], &state.archivers[idx+1], len);

    assert(state.numArchivers > 0);
    state.numArchivers--;
...
{
    while (state.numArchivers > 0)
    {
        if (!doDeregisterArchiver(state.numArchivers - 1))
            assert(!"nothing should be mounted during shutdown.");
    } /* while */

    allocator.Free(state.archivers);
    allocator.Free(state.archiveInfo);
    state.archivers = NULL;
...
static int doDeinit(void)
{
    closeFileHandleList(&state.openWriteList);
    ...
    if (state.baseDir != NULL)
    {
        allocator.Free(state.baseDir);
        state.baseDir = NULL;
    } /* if */

    if (state.userDir != NULL)
    {
        allocator.Free(state.userDir);
        state.userDir = NULL;
    } /* if */

    if (state.prefDir != NULL)
    {
        allocator.Free(state.prefDir);
        state.prefDir = NULL;
    } /* if */

    if (state.archiveInfo != NULL)
    {
        allocator.Free(state.archiveInfo);
        state.archiveInfo = NULL;
    } /* if */

    if (state.archivers != NULL)
    {
        allocator.Free(state.archivers);
        state.archivers = NULL;
    } /* if */

    state.allowSymLinks = 0;
    state.initialized = 0;
...
{
    BAIL_IF(!state.initialized, PHYSFS_ERR_NOT_INITIALIZED, 0);
    return doDeinit();
} /* PHYSFS_deinit */
...
static int doRegisterArchiver(const PHYSFS_Archiver *_archiver)
{
    ...
    const size_t len = (state.numArchivers + 2) * sizeof (void *);
    ...
    for (i = 0; i < state.numArchivers; i++)
    {
        if (PHYSFS_utf8stricmp(state.archiveInfo[i]->extension, ext) == 0)
    ...
    ptr = allocator.Realloc(state.archiveInfo, len);
    GOTO_IF(!ptr, PHYSFS_ERR_OUT_OF_MEMORY, regfailed);
    state.archiveInfo = (PHYSFS_ArchiveInfo **) ptr;

    ptr = allocator.Realloc(state.archivers, len);
    GOTO_IF(!ptr, PHYSFS_ERR_OUT_OF_MEMORY, regfailed);
    state.archivers = (PHYSFS_Archiver **) ptr;

    state.archiveInfo[state.numArchivers] = info;
    state.archiveInfo[state.numArchivers + 1] = NULL;

    state.archivers[state.numArchivers] = archiver;
    state.archivers[state.numArchivers + 1] = NULL;
...
{
    BAIL_IF(!state.initialized, PHYSFS_ERR_NOT_INITIALIZED, NULL);
    return (const PHYSFS_ArchiveInfo **) state.archiveInfo;
...
const char *PHYSFS_getPrefDir(const char *org, const char *app)
{
    ...
    allocator.Free(state.prefDir);
    state.prefDir = __PHYSFS_platformCalcPrefDir(org, app);
    BAIL_IF_ERRPASS(!state.prefDir, NULL);

    assert(strlen(state.prefDir) > 0);
    endstr = state.prefDir + (strlen(state.prefDir) - 1);
    assert(*endstr == dirsep);
    *endstr = '\0';  /* mask out the final dirsep for now. */

    if (!__PHYSFS_platformStat(state.prefDir, &statbuf, 1))
    {
        for (ptr = strchr(state.prefDir, dirsep); ptr; ptr = strchr(ptr+1, dirsep))
        {
            *ptr = '\0';
            __PHYSFS_platformMkDir(state.prefDir);
            *ptr = dirsep;
        } /* for */

        if (!__PHYSFS_platformMkDir(state.prefDir))
        {
            allocator.Free(state.prefDir);
            state.prefDir = NULL;
        } /* if */
    ...
    *endstr = dirsep;  /* readd the final dirsep. */
    ...
    return state.prefDir;
} /* PHYSFS_getPrefDir */


const char *PHYSFS_getBaseDir(void)
{
    return state.baseDir;   /* this is calculated in PHYSFS_init()... */
} /* PHYSFS_getBaseDir */


const char *__PHYSFS_getUserDir(void)  /* not deprecated internal version. */
{
    return state.userDir;   /* this is calculated in PHYSFS_init()... */
} /* __PHYSFS_getUserDir */
...
const char *PHYSFS_getWriteDir(void)
{
    ...
    if (state.writeDir != NULL)
        retval = state.writeDir->dirName;
...
int PHYSFS_setWriteDir(const char *newDir)
{
    ...
    if (state.writeDir != NULL)
    {
        BAIL_IF_ERRPASS(!freeDirHandle(state.writeDir, state.openWriteList),
                            0);
        state.writeDir = NULL;
    } /* if */
    ...
    if (newDir != NULL)
    {
        state.writeDir = createDirHandle(NULL, newDir, NULL, 1);
        retval = (state.writeDir != NULL);
    } /* if */
    return retval;
} /* PHYSFS_setWriteDir */
                   ...
                   const char *mountPoint, int appendToPath)
{
    ...
    for (i = state.searchPath; i != NULL; i = i->next)
    {
...
int PHYSFS_unmount(const char *oldDir)
{
    ...
    for (i = state.searchPath; i != NULL; i = i->next)
    {
        ...
        {
            next = i->next;
            BAIL_IF_ERRPASS(!freeDirHandle(i, state.openReadList),
                                0);

            if (prev == NULL)
                state.searchPath = next;
            else
                prev->next = next;
...
{
    state.allowSymLinks = allow;
} /* PHYSFS_permitSymbolicLinks */


int PHYSFS_symbolicLinksPermitted(void)
{
    return state.allowSymLinks;
...
static int doMkdir(const char *_dname, char *dname)
{
    ...
    std::unique_lock lock(state.lock);
    BAIL_IF(!state.writeDir, PHYSFS_ERR_NO_WRITE_DIR, 0);
    h = state.writeDir;
...
int PHYSFS_enumerate(const char *_fn, PHYSFS_EnumerateCallback cb, void *data)
{
    ...
    else
    {
       ...
       std::shared_lock lock(state.lock);
        ...
        for (i = state.searchPath; (retval == PHYSFS_ENUM_OK) && i; i = i->next)
        {
            char *arcfname = fname;
            ...
            else if (verifyPath(i, &arcfname, 0))
            {
                ...
                else if ((!state.allowSymLinks) && (i->funcs->info.supportsSymlinks))
                {
...
static PHYSFS_File *doOpenWrite(const char *_fname, int appending)
{
    ...
    if (sanitizePlatformIndependentPath(_fname, fname))
    {
        ...
        
        {
            std::shared_lock lock(state.lock);
            if(!state.writeDir) {
                if(PHYSFS_ERR_NO_WRITE_DIR) 
                    PHYSFS_setErrorCode(PHYSFS_ERR_NO_WRITE_DIR);
                __PHYSFS_smallFree(fname);
                return ((PHYSFS_File *) fh);
            }
            //GOTO_IF(!state.writeDir, PHYSFS_ERR_NO_WRITE_DIR, doOpenWriteEnd);
        ...
        else
        {
            ...
            fh->dirHandle = h;
            std::unique_lock lock(state.lock);
            fh->next = state.openWriteList;
            state.openWriteList = fh;
        } /* else */
    } /* if */
...
PHYSFS_File *PHYSFS_openRead(const char *_fname)
{
    ...
    if (sanitizePlatformIndependentPath(_fname, fname))
    {
        ...
        //GOTO_IF(!state.searchPath, PHYSFS_ERR_NOT_FOUND, openReadEnd);
        if(!state.searchPath) {
             if(PHYSFS_ERR_NOT_FOUND)
        ...
        fh->dirHandle = i;
        std::unique_lock lock(state.lock);
        fh->next = state.openReadList;
        state.openReadList = fh;
...
int PHYSFS_close(PHYSFS_File *_handle)
{
    ...
    rc = closeHandleInOpenList(&state.openReadList, handle);
    BAIL_IF_ERRPASS(rc == -1, 0);
    if (!rc)
    {
        rc = closeHandleInOpenList(&state.openWriteList, handle);
        BAIL_IF_ERRPASS(rc == -1, 0);
    } /* if */
...
int PHYSFS_stat(const char *_fname, PHYSFS_Stat *stat)
{
    ...
    if (sanitizePlatformIndependentPath(_fname, fname))
    {
        if (*fname == '\0')
        {
            ...
            stat->readonly = !state.writeDir; /* Writeable if we have a state.writeDir */
            retval = 1;
        } /* if */
        else
        {
            ...
            int exists = 0;
            std::shared_lock lock(state.lock);
            for (i = state.searchPath; ((i != NULL) && (!exists)); i = i->next)
            {
                char *arcfname = fname;
